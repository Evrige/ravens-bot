...
import { resetHivePanel } from "../../services/upsertHivePanel"; // ✅ ДОБАВЬ
...
export const organisationAddCommand = {
	data: new SlashCommandBuilder()
		...
		.addStringOption(option =>
			option.setName("name").setDescription("Название").setRequired(true)
		)
		.addStringOption(option =>
			option
				.setName("type")
	...
	async execute(interaction: ChatInputCommandInteraction) {
		...
		if (exists) return interaction.editReply("❌ Такая организация уже существует.");

		const forumId = getForumIdByType(type);
		if (!forumId) return interaction.editReply("❌ Forum ID не задан в env.");
		...
		// 2) создаём тред в форуме под эту организацию
		...
		try {
			...
			return interaction.editReply(`❌ Не удалось создать тред в форуме: ${e?.message ?? e}`);
		}

		// 3) сохраняем thread.id в channelId
		await prisma.organisation.update({
			where: { id: org.id },
			data: { channelId: thread.id },
		});

		// ✅ 4) обновляем (перерисовываем) панель, чтобы новые организации появились в селекте
		const refreshed = await resetHivePanel(interaction.client);

		return interaction.editReply(
			`✅ Организация **${name}** создана.\n` +
			`📌 В форуме создан тред: **${thread.name}** (channelId сохранён).\n` +
			(
				refreshed.ok
