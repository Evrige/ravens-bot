...
export const FACTION_PANEL = {
	THREAD_NAME: "📋 Список фракций",
	BOTMSG_TYPE: "faction_list_panel",
	MULTI_PREFIX: "faction_list_panel_chunk_",

	customId: {
		edit: (orgId: bigint) => `faction:list:edit:${orgId.toString()}`,
		freeze: (orgId: bigint) => `faction:list:freeze:${orgId.toString()}`,
		del: (orgId: bigint) => `faction:list:delete:${orgId.toString()}`,
		editModal: (orgId: bigint) => `faction:list:modal_edit:${orgId.toString()}`,
	},
} as const;
...
function buildFactionsChunk(
	...
	const title =
		totalChunks > 1
			? `## 🛡️ Фракции (${chunkIndex + 1}/${totalChunks})`
			: "## 🛡️ Фракции";

	const container: any = {
		type: V2.Container,
		components: [
			{ type: V2.TextDisplay, content: title },
			{ type: V2.Separator },
		],
	};
			...
			components: [
				{
					...
					style: 2,
					label: "Редактировать",
					custom_id: FACTION_PANEL.customId.edit(org.id),
				},
				{
					type: 2,
					style: org.isFreeze ? 3 : 2,
					label: org.isFreeze ? "Разморозить" : "Заморозить",
					custom_id: FACTION_PANEL.customId.freeze(org.id),
				},
				{
					type: 2,
					style: 4,
					label: "Удалить",
					custom_id: FACTION_PANEL.customId.del(org.id),
				},
			],
		});
...
async function getFactionForum(client: Client): Promise<ForumChannel | null> {
	const forumId = config.DB_FORUM_FRACTION_ID;
...
async function ensureThread(forum: ForumChannel, storedChannelId?: string) {
	...
	const foundActive = active?.threads?.find((t) => t.name === FACTION_PANEL.THREAD_NAME);
	if (foundActive) {
	...
	const foundArchived = archived?.threads?.find((t) => t.name === FACTION_PANEL.THREAD_NAME);
	if (foundArchived) {
	...
	return await forum.threads.create({
		name: FACTION_PANEL.THREAD_NAME,
		message: { content: "Служебный тред панели фракций." },
		...(appliedTags ? { appliedTags } : {}),
	});
}

function getChunkType(index: number) {
	return `${FACTION_PANEL.MULTI_PREFIX}${index}`;
}
...
async function getStoredChunkMessages() {
	const rows = await prisma.botMessage.findMany({
		where: {
			OR: [
				{ type: FACTION_PANEL.BOTMSG_TYPE },
				{ type: { startsWith: FACTION_PANEL.MULTI_PREFIX } },
			],
		},
		orderBy: { type: "asc" },
	});
	...
	return rows
		.map((row) => {
			if (row.type === FACTION_PANEL.BOTMSG_TYPE) {
				return { index: 0, type: row.type, messageId: row.messageId, channelId: row.channelId };
			}

			const n = Number(row.type.replace(FACTION_PANEL.MULTI_PREFIX, ""));
			if (Number.isNaN(n)) return null;

			return { index: n, type: row.type, messageId: row.messageId, channelId: row.channelId };
		})
		.filter(Boolean)
...
async function saveChunkMessage(index: number, message: Message, threadId: string) {
	const type = index === 0 ? FACTION_PANEL.BOTMSG_TYPE : getChunkType(index);

	await prisma.botMessage.upsert({
		where: { type },
		create: { type, messageId: message.id, channelId: threadId },
		update: { messageId: message.id, channelId: threadId },
	});
}

async function deleteStoredChunk(index: number) {
	const type = index === 0 ? FACTION_PANEL.BOTMSG_TYPE : getChunkType(index);
	await prisma.botMessage.delete({ where: { type } }).catch(() => null);
}
...
async function syncFactionChunkMessages(thread: ThreadChannel, orgs: FactionOrg[]) {
	...
	for (let i = 0; i < finalChunks.length; i++) {
		const components = buildFactionsChunk(finalChunks[i], i, finalChunks.length) as any;
	...
	await prisma.botMessage.deleteMany({
		where: { type: getChunkType(0) },
	}).catch(() => null);

	const firstRow = await prisma.botMessage.findUnique({
		where: { type: FACTION_PANEL.BOTMSG_TYPE },
	});
	...
	return { edited, created, deleted, total: finalChunks.length };
}

export async function upsertFactionListPanel(client: Client) {
	try {
		const forum = await getFactionForum(client);
		if (!forum) {
			return {
				ok: false as const,
				reason: "DB_FORUM_FRACTION_ID not set or not a forum",
			};
		}
		...
		const syncResult = await syncFactionChunkMessages(thread, orgs);
